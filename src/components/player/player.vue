<template>
    <div class="player" v-show="playlist.length">
        ...
        <audio ref="audio" :src="crruntSongUrl" @canplay='ready' @error='error' @timeupdate='updateTime' @ended="end" ></audio>
    </div>
</template>
<script>
...
export default {
  ...
  watch: {
    currentSong(newSong, oldsong) {
      ...
      this.currentLyric && this.currentLyric.stop();
      newSong.url().then(songUrl => {
        this.crruntSongUrl = songUrl;
      });
      this.getLyric();
    },
    playing(newPlaying) {
      this.$nextTick(() => {
        ...
        if (newPlaying && this.crruntSongUrl) {
          audio.play();
        } else {
          audio.pause();
        }
